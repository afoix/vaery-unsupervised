...
class ResNet18Enc(nn.Module):
    ...
    def __init__(self, num_Blocks=[2,2,2,2], z_dim=10, nc=3): #z_dims will go to 1, nc = 3, 
        ...
        self.final_conv = nn.Conv2d(512*2, 2 * z_dim, kernel_size=3, stride = 2)
    ...
    def forward(self, x):
        ...
        x = x.view(b,c*h*w)
        print(x.shape)
        linear_layer = nn.Linear(c*h*w, c)
...
class ResNet18Dec(nn.Module):
    ...
    def _make_layer(self, BasicBlockDec, planes, num_Blocks, stride):
        ...
        layers = []
        print(f'{planes}_planesbefore stride loop')
        print(f'{self.in_planes}_inplanesbefore stride loop')        
        for stride in reversed(strides):
            layers += [BasicBlockDec(self.in_planes, stride)]
            print(f'{self.in_planes} planesafter stride loop')
            print(f'{planes} inplanesafter stride loop')            
        self.in_planes = planes
        print(f'{planes} planesafter redefining in_planes')   
